...
public class SEPAReaderPain_008_001_02 implements SEPAReader {
	...
	public SEPAReaderPain_008_001_02(){
		super();
		log = LogFactory.getLog(SEPAReaderPain_008_001_02.class);
	}
	...
	public SEPAMessage parse(InputStream is) throws SEPAException {
		...
		try {
			...
			// alle PmtInf holen.
			List<PaymentInstructionInformation4> debList = doc.getCstmrDrctDbtInitn().getPmtInf();
			for (PaymentInstructionInformation4 payment : debList) {
				...
				if(! (payment.getCdtrSchmeId() == null)){
					dt.setCreditorId(payment.getCdtrSchmeId().getId().getPrvtId().getOthr().get(0).getId());
				}
				...
				// Alle Transaktionen
				List<DirectDebitTransactionInformation9> dtil = payment.getDrctDbtTxInf();
				for (DirectDebitTransactionInformation9 dti : dtil) {
					...
					if(dti.getRmtInf() != null){
						ti.setRemitanceInformation(dti.getRmtInf().getUstrd().get(0));
					}
					if(dti.getPurp() != null){
					...
					if(dt.getCreditorId() == null){
						if(dti.getDrctDbtTx().getCdtrSchmeId() != null){
							dt.setCreditorId(dti.getDrctDbtTx().getCdtrSchmeId().getId().getPrvtId().getOthr().get(0).getId() );
						}
					}
